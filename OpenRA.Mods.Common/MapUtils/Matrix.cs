...
namespace OpenRA.Mods.Common.MapUtils
{
	...
	public sealed class Matrix<T>
	{
		...
		/// <summary>Get the value at xy if it is in bounds, else return a fallback value.</summary>
		public T GetOrDefault(int2 xy, T fallback)
			=> ContainsXY(xy) ? this[xy] : fallback;

		/// <summary>Set the value at xy if it is in bounds, else do nothing.</summary>
